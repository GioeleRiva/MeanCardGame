...
public class JoinScreen extends Pane {
	...
	public JoinScreen() {
		...
		Card wCard = new Card(Main.roomCode, "white", 0);
		...
		Keyboard keyboard = new Keyboard(whiteCard, 4);
		this.getChildren().add(keyboard);
		Button back = new Button("Back.", "playScreen");
				...
				scaleTransition1.setOnFinished(play2 -> {
					...
					scaleTransition2.setOnFinished(play3 -> {
						...
						scaleTransition3.setOnFinished(done -> {
							...
							Thread thread = new Thread(new Listener("185.58.194.230", 15000, Main.playerName, false,
